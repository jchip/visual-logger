...
class VisualLogger {
  ...
  _isDefaultSpin(options) {
    return options.spinInterval === DEFAULT_SPINNER_INTERVAL;
  }

  _isAllDefaultSpinItemsStop() {
    return Object.keys(this._itemOptions)
      .concat(Object.getOwnPropertySymbols(this._itemOptions))
      .filter(k => this._isDefaultSpin(this._itemOptions[k]))
      .every(k => {
    ...
    if (!this._spinTimer && this._shouldLogItem()) {
      this._spinTimer = setInterval(() => {
        ...
        Object.keys(this._itemOptions)
          ...
          .forEach(name => {
            const options = this._itemOptions[name];
            if (options._spinning && this._isDefaultSpin(options)) {
  ...
  _stopSpinTimer(force = false) {
    if (force || this._isAllDefaultSpinItemsStop()) {
