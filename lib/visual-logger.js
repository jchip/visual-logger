...
class VisualLogger {
  ...
  updateItem(name, data) {
    ...
    if (this._shouldLogItem()) {
      ...
      this._lines[x] = this._renderLine(options);
      this._renderOutput();
    } else if (this._itemType === LogItemTypes.simple) {
      this._dots = true;
      this._output.write(".");
    }
  ...
  clearItems() {
    ...
    } else if (this._itemType === LogItemTypes.simple && this._dots) {
      this._dots = false;
      this._output.write("\n");
    }
    return this;
  ...
  freezeItems(showItems) {
    for (const name in this._itemOptions) {
      ...
      if (opt.spinTimer) {
        clearInterval(opt.spinTimer);
        opt.spinTimer = undefined;
      }
    }
    this._output.visual.clear();
